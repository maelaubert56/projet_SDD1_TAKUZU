...
int** generer_masque(int size,int niveau){
    ...
    int nb_case;
    switch(niveau){
        case 1: nb_case = (int)(size*size*0.6); break;
        case 2: nb_case = (int)(size*size*0.5); break;
        case 3: nb_case = (int)(size*size*0.4); break;
    }
    while (nb_case != 0) {
        for (i = 0; i < size; i++) {
            for (j = 0; j < size; j++) {
...
void resoudre(int **masque, int** solution, int size){
    ...
    while (gagne != 1 && vies != 0){
        ...
        if(valide==1){
            ...
            switch(valide){
                case -1: printf("La ligne %d contient deja %d fois la valeur %d...",coords.i,size/2,val);break;
                case -2: printf("La colonne %d contient deja %d fois la valeur %d...",coords.j,size/2,val);break;
                case -3: printf("Placer un %d ici provoquerai une suite de plus de 2 fois la valeur %d...",val,val);break;
                case -4: printf("Placer un %d ici provoquerai une suite de plus de 2 fois la valeur %d...",val,val);break;
            }
